...
export function TopBar() {
  ...
  return (
    <div className='flex items-center gap-4 px-4 md:px-6 h-14 bg-slate-900/80 backdrop-blur-md border-b border-slate-800'>
      {/* Breadcrumb */}
      <div className='hidden md:flex items-center gap-1.5 text-sm min-w-0 flex-1'>
        ...
        {breadcrumbs.map((crumb, i) => (
          <span key={i} className='flex items-center gap-1.5'>
            {i > 0 && <ChevronRight size={13} className='text-slate-600 shrink-0' />}
            <span
              className={
        ...
        <AnimatePresence>
          {searchOpen ? (
            ...
            <motion.button
              ...
              className='flex items-center gap-2 w-full bg-slate-800/50 border border-slate-700/50 rounded-lg px-3 py-1.5 text-sm text-slate-500 hover:border-slate-600 hover:text-slate-400 transition-colors'
            >
              <Search size={14} />
              <span>Search...</span>
              <kbd className='ml-auto text-[10px] text-slate-600 border border-slate-700 rounded px-1 py-0.5 font-mono'>
                Ctrl+K
              </kbd>
            </motion.button>
          )}
      ...
      <div className='flex items-center gap-1.5'>
        ...
        <div className='relative'>
          <motion.button
            ...
            title='Notifications'
          >
            {unreadCount > 0 ? <BellDot size={16} className='text-sky-400' /> : <Bell size={16} />}
            {unreadCount > 0 && (
          ...
          <AnimatePresence>
            {notifOpen && (
              <motion.div
                ...
                className='absolute right-0 top-10 w-80 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl overflow-hidden z-50'
              >
                ...
                <div className='divide-y divide-slate-800'>
                  {mockNotifications.map((n) => (
                    ...
                    >
                      <p className={`leading-relaxed ${n.unread ? 'text-slate-200' : 'text-slate-400'}`}>
                        {n.text}
                      </p>
                      <p className='text-slate-600 mt-1'>{n.time}</p>
                    </div>
                  ))}
                </div>
